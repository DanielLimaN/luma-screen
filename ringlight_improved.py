...
class RingLightOverlay(QWidget):
    ...
    def paintEvent(self, event) -> None:
        ...
        This method draws a multi-layered gradient effect around all edges of the
        screen. The effect uses screen blend mode for a soft, natural-looking light.
        Multiple layers create a bloom effect that gradually fades toward the center.
        
        Args:
            event: The paint event (provided by Qt).
        """
        painter = QPainter(self)
        
        # Use screen composition mode for a soft, additive light effect
        painter.setCompositionMode(QPainter.CompositionMode_Screen)
        
        width = self.width()
        height = self.height()
        
        # Limit thickness to screen dimensions to prevent rendering issues
        effective_thickness = min(
            self.thickness,
            max(RingLightConfig.MIN_THICKNESS, min(width, height))
        )
        
        base_color = QColor(self.color)
        
        # Render multiple layers for a realistic bloom effect
        for intensity, size_multiplier in RingLightConfig.BLOOM_LAYERS:
            layer_size = max(1, int(effective_thickness * size_multiplier))
            
            # Calculate alpha channel with safety limits
            alpha = int(255 * self.brightness * intensity)
            alpha = max(0, min(RingLightConfig.MAX_ALPHA, alpha))
            
            # Create colored layer
            layer_color = QColor(base_color)
            layer_color.setAlpha(alpha)
            
            # Create transparent color for gradient
            transparent_color = QColor(base_color)
            transparent_color.setAlpha(0)
            
            # Render gradients on all four edges
            self._render_top_gradient(painter, width, layer_size, layer_color, transparent_color)
            self._render_bottom_gradient(painter, width, height, layer_size, layer_color, transparent_color)
            self._render_left_gradient(painter, height, layer_size, layer_color, transparent_color)
            self._render_right_gradient(painter, width, height, layer_size, layer_color, transparent_color)
        
        painter.end()
    
    def _render_top_gradient(
        self,
        painter: QPainter,
        width: int,
        size: int,
        ...
        """Render gradient on the top edge."""
        gradient = QLinearGradient(0, 0, 0, size)
        gradient.setColorAt(0, color)
        gradient.setColorAt(1, transparent)
        painter.fillRect(QRect(0, 0, width, size), gradient)
    ...
    def _render_bottom_gradient(
        ...
        """Render gradient on the bottom edge."""
        gradient = QLinearGradient(0, height, 0, height - size)
        gradient.setColorAt(0, color)
        gradient.setColorAt(1, transparent)
        painter.fillRect(QRect(0, height - size, width, size), gradient)
    
    def _render_left_gradient(
        self,
        painter: QPainter,
        height: int,
        size: int,
        ...
        """Render gradient on the left edge."""
        gradient = QLinearGradient(0, 0, size, 0)
        gradient.setColorAt(0, color)
        gradient.setColorAt(1, transparent)
        painter.fillRect(QRect(0, 0, size, height), gradient)
    
    def _render_right_gradient(
        self,
        painter: QPainter,
        width: int,
        height: int,
        ...
        """Render gradient on the right edge."""
        gradient = QLinearGradient(width, 0, width - size, 0)
        gradient.setColorAt(0, color)
        gradient.setColorAt(1, transparent)
        painter.fillRect(QRect(width - size, 0, size, height), gradient)
...
class ControlPanel(QMainWindow):
    ...
    def _create_action_buttons(self) -> QHBoxLayout:
        ...
        self.toggle_btn = QPushButton("Turn Off Overlay")
        self.panic_btn = QPushButton("PANIC: Turn Off Overlay")
        self.close_btn = QPushButton("Close All")
        
        row.addWidget(self.toggle_btn)
        row.addWidget(self.panic_btn)
    ...
    def _update_color_preview(self) -> None:
        """Update the color preview widget to show the current ring light color."""
